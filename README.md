...
## 1. 비트 시프트의 기본 개념
- **왼쪽 시프트(<<)**  
  - 오른쪽 빈 칸은 **항상 0**으로 채워진다.  
  - 값의 크기를 2배로 키우는 효과.

- **오른쪽 시프트(>>)**  
  - 어떤 값이 채워지느냐에 따라 **논리 시프트**와 **산술 시프트**로 나뉜다.
...
- 빈 자리를 **항상 0**으로 채운다.
- 부호와 무관하게 단순히 비트를 이동.
- 주로 **unsigned 타입**에서 사용.

---

## 3. 산술 시프트(Arithmetic Shift)
- 부호비트(Sign bit)를 유지하기 위해 빈 자리를 **기존 최상위 비트(MSB)** 값으로 채운다.
  - MSB가 0이면 0으로 채움  
  - MSB가 1이면 1로 채움
- 즉, **부호가 있는 정수(signed)**에서 사용.
- 이유: 2의 보수 표현을 쓰는 정수에서 부호가 바뀌지 않도록 하기 위해.

---

## 4. 왜 signed 오른쪽 시프트는 1이 채워질 수 있을까?
- 대부분의 시스템은 정수를 **2의 보수(Two’s Complement)**로 표현한다.
- 음수는 MSB가 1이며, 산술 시프트는 이 MSB를 유지하여 **부호를 보존**하려고 한다.
- 따라서 signed 값에서 1이 MSB라면 오른쪽 시프트할 때 빈자리가 **1로 채워진다**.
...
- `>>` 연산 시  
  ...
  - **signed → 산술 시프트 → 부호비트 유지(0 또는 1)**  
- 이는 **2의 보수 체계에서 정수의 부호를 유지하기 위한 방식**이다.
...
## 2025-10-13
 - TestApp 수정
   정수 N과 이후 N개의 정수 데이터를 입력받아 최솟값, 최댓값, 평균값, 최빈값, 짝수/홀수 개수, 절댓값 평균을 계산
